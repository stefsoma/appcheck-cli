  ...
  try {
    ...
    const answers = await inquirer.prompt([
      ...
      },
      {
        ...
        default: existingConfig.languageCodeFormat || 'en-US',
      },
      {
        type: 'checkbox',
        name: 'languages',
      ...
      },
      {
        ...
        message: 'Enter language mapping (e.g., "no:nb_NO,nn_NO;en:en_US,en_GB"):',
        default: existingConfig.languageMapping ? Object.entries(existingConfig.languageMapping).map(([key, value]) => `${key}:${Array.isArray(value) ? value.join(',') : value}`).join(';') : '',
        filter: (input) => {
          if (!input) return {};
          const mapping = {};
          input.split(';').forEach(pair => {
            ...
            if (key && values) {
              mapping[key.trim()] = values.split(',').map(v => v.trim());
            }
          });
          return mapping;
        }
